...
class TestAPI(unittest.TestCase):
    ...
    def test_anonymize_dicom_with_phi(self):
        ...
        try:
            ...
            self.assertEqual(str(downloaded.PatientName), "ANONYMIZED")
            self.assertEqual(str(downloaded.PatientID), "REDACTED")
            self.assertEqual(str(downloaded.PatientBirthDate), "19000101")
            self.assertEqual(str(downloaded.ReferringPhysicianName), "ANONYMIZED")
            self.assertEqual(str(downloaded.InstitutionName), "REDACTED")
