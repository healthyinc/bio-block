...
def detect_clinical_phi(text: str) -> Dict[str, int]:
    ...
    Kept as public API: services.safe_harbor calls it to count identifiers in
    a field without ever seeing the values. It now counts what the NER chain
    finds instead of what the Presidio analyzer found, and it still fails
    closed - a detector that cannot run raises rather than reporting nothing.
    """
